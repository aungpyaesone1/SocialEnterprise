...
function checkMember($email,$password){
    include("config/conf.php");
...
function insertUser($name,$email,$password,$code){
    include('config/conf.php');
    $query="insert into staff (name,email,password,code) values (:name,:email,:password,:code)";
    $stmt=$con->prepare($query);
    $stmt->execute(array(':name'=>$name,':email'=>$email,':password'=>$password,':code'=>$code));
    return $stmt->rowCount();
}
function selectUsers(){
    include('config/conf.php');
    $query = "select s.id,s.name,p.position_name from staff as s,positions as p where s.position_id=p.id";
    $stmt=$con->prepare($query);
    $stmt->execute();
...
function makeActive($code){
    include('config/conf.php');
    $query ="update staff set active=1 where code=:code";
    $stmt=$con->prepare($query);
...
function insertPost($staff_id,$title,$description,$date){
   include('config/conf.php');
   $query = "insert into posts (staff_id,title,description,created_at) values (:id,:title,:des,:date)";
    $stmt=$con->prepare($query);
    $stmt->execute(array(':id'=>$staff_id,':title'=>$title,':des'=>$description,':date'=>$date));
    return $stmt->rowCount();
}
function selectPosts(){
    include('config/conf.php');
    $query = "select p.id,p.title,p.description,p.created_at,s.name from posts as p,staff as s where s.id=p.staff_id ORDER BY created_at DESC";
    $stmt = $con->prepare($query);
    $stmt->execute();
    return $stmt->fetchAll(PDO::PARAM_STR);
}
function increaseLike($post_id,$staff_id){
    include('config/conf.php');
    $query = "insert into post_likes (post_id,staff_id) values (:post_id,:staff_id)";
    $stmt=$con->prepare($query);
    $stmt->execute(array(':post_id'=>$post_id,':staff_id'=>$staff_id));
    return $stmt->rowCount();
}
function selectLikes($id){
    include('config/conf.php');
    $query = "select count(*),post_id from post_likes where post_id=:id";
    $stmt = $con->prepare($query);
...
function checkLike($id){
    include('config/conf.php');
    $query="select staff_id from post_likes where post_id=:id";
    $stmt=$con->prepare($query);
    $stmt->bindParam(':id',$id,PDO::PARAM_STR);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
function decreaseLike($post_id,$staff_id){
    include('config/conf.php');
    $query = "delete from post_likes where post_id=:post_id and staff_id=:staff_id";
    $stmt=$con->prepare($query);
    $stmt->execute(array(':post_id'=>$post_id,':staff_id'=>$staff_id));
    return $stmt->rowCount();
}
function insertComment($post_id,$staff_id,$text){
    include('config/conf.php');
    $query = "insert into posts_comments (comment,post_id,staff_id) values (:comment,:post_id,:staff_id)";
    $stmt = $con->prepare($query);
    $stmt->execute(array(':comment'=>$text,':post_id'=>$post_id,':staff_id'=>$staff_id));
    return $stmt->rowCount();
}
function selectComments($post_id){
    include('config/conf.php');
    $query = "select c.comment,s.name from staff as s,posts_comments as c where s.id=c.staff_id and c.post_id=$post_id";
    $stmt = $con->prepare($query);
    $stmt->execute();
    return $stmt->fetchAll(PDO::PARAM_STR); 
}
function selectCommentCount($id){
    include('config/conf.php');
    $query = "select count(*) from posts_comments where post_id=:id";
    $stmt = $con->prepare($query);
    $stmt->bindParam(':id',$id,PDO::PARAM_STR);
    $stmt->execute();
    return $stmt->fetch(PDO::FETCH_ASSOC);
}
function selectPostsWithId($id){
    include('config/conf.php');
    $query = "select p.id,p.title,p.description,p.created_at,s.name from posts as p,staff as s where s.id=p.staff_id and p.staff_id=:id ORDER BY created_at desc";
    $stmt = $con->prepare($query);
    $stmt->bindParam(':id',$id,PDO::PARAM_STR);
    $stmt->execute();
    return $stmt->fetchAll(PDO::PARAM_STR);
...
function updateUser($id,$name,$email,$profile,$position){
    include('config/conf.php');
    $query ="update staff set name='$name',email='$email',profile='$profile',position_id='$position' where id=:id";
    $stmt=$con->prepare($query);
    $stmt->bindParam(':id',$id,PDO::PARAM_STR);
    $stmt->execute();
    return $stmt->rowCount();
...
/*$email="aungpyaesone107@gmail.com";
mailSend($email);*/
function mailSend($email){
    ...
    $mail->Username = "aungpyaesone.aps45@gmail.com";                 
    $mail->Password = "123456shin";                           
    //If SMTP requires TLS encryption then set it
    //$mail->SMTPSecure = "tls";                           
    //Set TCP port to connect to 
    $num_str = sprintf("%06d", mt_rand(1, 999999));
    $mail->Port = 587;                                   
    $mail->From = "aungpyaesone.aps45@gmail.com";
    $mail->FromName = "Yammobots";
    ...
    $mail->Subject = "Email Verification(Yammobot.com)";
    $mail->Body = "<i><b>Your activation code is</i><b> ".$num_str;
